...
import type { CalendarDensitySignal } from "./signals/calendarDensity.ts";
import type { RawCalendarEvent } from "./signals/calendarDensity.ts";
import type { SleepSignal } from "./signals/sleepSummary.ts";
import type { RawSleepRecord } from "./signals/sleepSummary.ts";
...
export async function fetchGroups(
  ...
  const q = supabase.from("groups").select(GROUP_SELECT).limit(cap);
  if (order === "name_asc") {
