...
import { createAdminClient, hashDeviceSecret, jsonResponse, RELAY_CORS_HEADERS } from "../_shared/relayAuth.ts";
...
async function createPairingCode(ownerId, adminClient) {
...
async function exchangePairingCode(adminClient, input) {
  const code = input.code.trim().toUpperCase();
  if (!code || !input.deviceName?.trim() || !input.deviceSecret) {
    throw new Error("missing code, deviceName, or deviceSecret");
  }
  const { data: pairingRow, error: fetchError } = await adminClient.from("relay_pairing_codes").select("id, owner_id, expires_at, consumed_at").eq("code", code).maybeSingle();
  if (fetchError) {
  ...
  const { data: device, error: deviceError } = await adminClient.from("relay_devices").insert({
  ...
  const { error: consumeError } = await adminClient.from("relay_pairing_codes").update({
    consumed_at: new Date().toISOString()
  }).eq("id", pairingRow.id).is("consumed_at", null);
  if (consumeError) {
...
export async function handler(req) {
  ...
  if (body.action === "exchange") {
    try {
      ...
      if (message.includes("invalid") || message.includes("expired") || message.includes("already used") || message.includes("missing")) {
        return jsonResponse(400, {
          error: message
        });
      }
