// outlook-oauth Edge Function — exchange Microsoft OAuth 2.0 PKCE code for
// tokens and persist to user_provider_tokens.
//
// Deploy:
...
async function exchangeCodeForTokens(input: {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Microsoft token exchange failed (${response.status}): ${text.slice(0, 200)}`,
    );
  }
...
async function fetchMicrosoftAccountId(accessToken: string): Promise<string> {
  ...
  if (!response.ok) {
    const text = await response.text();
    throw new Error(
      `Microsoft Graph /me failed (${response.status}): ${text.slice(0, 200)}`,
    );
  }
  const data = await response.json();
  const accountId = data.id;
  if (!accountId) {
    throw new Error("Microsoft Graph /me returned no user id");
  }
...
Deno.serve(async (req) => {
  ...
  if (!CLIENT_ID || !CLIENT_SECRET) {
    return jsonResponse(500, {
      error: "Microsoft client credentials not configured",
    });
  }
  ...
  try {
    ...
    const accountId = await fetchMicrosoftAccountId(tokens.accessToken);
