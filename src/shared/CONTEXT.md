...
- **Goals & Values**: long-running, User-authored, edited explicitly when the User decides to add or change one. "Be more present with family." Not inferred (except via **Values Discovery** — per **ADR-0011**).
...
**Values Discovery**:
The web onboarding flow that helps a User seed **Goals & Values** by swiping on media characters (align / reject), ranking their aligned set, and optionally accepting AI-proposed goal statements. Swipe and rank decisions persist as `character_values_alignment` rows; `suggest-values-characters` and `infer-values-from-alignments` Edge Functions are **read-only for G&V** — they return character or goal **proposals** only. The User confirms, edits, or deselects proposals in `_ValuesConfirmPanel` before any write to Goals & Values via `userContext.addGoal`. This is the **sole exception** to "Goals & Values are not inferred": AI proposes, User must confirm. Per **ADR-0011**.
_Avoid_: Values quiz, personality test (this is a guided authoring aid with an explicit confirm gate, not a scored assessment).
