...
1. **Ambient Intelligence** (existing — see CONTEXT.md and the `ambient-pass` Edge Function). Continuously runs Agent Passes per Relationship; emits Candidate Sets the User accepts/edits/declines. The agent never auto-executes — every effect on the world passes through a User pick.
...
The three agents share a single LLM tool surface but have non-overlapping write rights, encoded as separate tool sets. The Conversational and Extraction agents each get their own Edge Function (`chat-respond`, `extract-context`); Ambient continues to use `ambient-pass`.
...
- **`/talk` voice infrastructure preserved.** The Whisper STT and ElevenLabs TTS adapters, browser MediaRecorder pipeline, and `voice-stt`/`voice-tts` Edge Functions stay. PR 4 unifies them into the new `/agent` composer (mic button → STT → assistant turn → optional TTS playback). On mobile, voice now routes through the Chat tab (`MobileChatScreen` mic → STT → composer → `chat-respond`); see the 2026-05-22 amendment below.
- **Web + mobile, peer surfaces (amended 2026-05-21).** The Conversational and Extraction surfaces ship on **both web and mobile** — they are peer surfaces backed by the same `chat-respond` and `extract-context` Edge Functions and the same `chats` / `chat_messages` tables. The original "web-only for v1" line in this ADR is **retracted** — re-reading mobile's purpose with the project sponsor, the mobile app's primary value is voice-first context capture into the Conversational surface (the moment when life context is freshest is in-pocket, not at a desk), and gating that behind a v2 ADR was the wrong call. Web remains text-primary; mobile is voice-primary with TTS-default playback of assistant turns.
- **User-initiated Engaged Pass retired on mobile (amended 2026-05-22).** The original "Engaged Pass continues to coexist on mobile" lines are **retracted**. Migration `20260530000004_remove_engaged_pass_mode` removes `engaged` from scheduled passes; `VoiceSessionManager.onUserTurn` rejects. Mobile "Talk to Claude" entry points navigate to the Chat tab with a relationship-scoped starter prompt rather than opening a Pass surface. Ambient Intelligence continues via baseline and triggered passes only.
- **No retirement of Ambient Intelligence.** Despite the URL collision implied by ADR-0008's *"`/agent` (text+voice chat, replaces `/talk`)"*, Ambient Intelligence is unchanged — it runs on schedule and surfaces Candidate Sets the User picks from. `/agent` is the **Conversational Intelligence** surface (free-form, global, read-only). It does not overlap with Ambient; they answer different questions.
